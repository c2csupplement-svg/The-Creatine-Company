...
const FAQS = {
  ...
  fa: [
    {
      q: "آیا کراتین باعث می‌شود ظاهری نرم، پف‌کرده یا متورم داشته باشم؟",
      a: "خیر. این یک برداشت اشتباه درباره نحوه عملکرد آب‌رسانی است. کراتین آب را به داخل سلول‌های عضلانی شما می‌کشد، نه زیر پوست. این آب‌رسانی سلولی باعث می‌شود عضلات شما پرتر، سفت‌تر و متراکم‌تر به نظر برسند. اگر در دو هفته اول حدود یک کیلوگرم به وزن شما اضافه شود، این افزایش معمولاً مربوط به آب موجود در بافت عضلات است، نه چربی. این موضوع نشان می‌دهد که محصول در حال انجام عملکرد مورد انتظار خود است.",
    },
    {
      q: "چرا کراتین قدیمی من ته لیوان می‌نشیند و مزه‌ای شبیه شن دارد؟",
      a: "زیرا کراتین‌های معمولی اغلب به اندازه کافی ریز آسیاب نشده‌اند. برخی برندهای قدیمی از پودر ارزان ۸۰ مش استفاده می‌کنند که به‌خوبی در آب سرد حل نمی‌شود و در کف شیکر باقی می‌ماند. ما از کراتین میکرونایز فوق‌العاده ریز ۲۰۰ مش استفاده می‌کنیم. این نوع کراتین سطح تماس بیشتری دارد و به همین دلیل سریع‌تر حل می‌شود، رسوب گچی باقی نمی‌گذارد و به‌راحتی در آب، آبمیوه یا نوشیدنی قبل از تمرین مصرف می‌شود.",
    },
    {
      q: "آیا می‌توانم این ساشه را مستقیماً با قهوه یا نوشیدنی قبل از تمرین مخلوط کنم؟",
      a: "بله. ساشه‌های ما حاوی ۱۰۰٪ کراتین مونوهیدرات خالص و بدون طعم هستند و هیچ شیرین‌کننده مصنوعی یا رنگی ندارند. می‌توانید ساشه را باز کرده و مستقیماً به قهوه صبحگاهی، پروتئین وی یا نوشیدنی قبل از تمرین خود اضافه کنید، بدون اینکه طعم آن تغییر کند. برخلاف باورهای قدیمی درباره تداخل کافئین و کراتین، می‌توان این دو را در کنار هم مصرف کرد؛ البته باید در طول روز آب کافی بنوشید.",
    },
    {
      q: "من فقط ۴ روز در هفته تمرین می‌کنم. آیا باید در روزهای استراحت مصرف ساشه را متوقف کنم؟",
      a: "هرگز مصرف روزانه را متوقف نکنید. کراتین یک محرک نیست و برای ایجاد افزایش فوری انرژی مصرف نمی‌شود. عملکرد آن بر اساس اصل اشباع است. هدف این است که سلول‌های عضلانی شما همیشه سطح مناسبی از کراتین داشته باشند تا به ریکاوری و بازسازی ATP کمک شود. اگر در روزهای استراحت مصرف آن را متوقف کنید، سطح اشباع به‌تدریج کاهش پیدا می‌کند. بنابراین هر روز یک ساشه مصرف کنید، چه تمرین سنگین داشته باشید و چه در خانه استراحت کنید.",
    },
    {
      q: "چگونه یک بسته ۹ درهمی می‌تواند با برندهای قدیمی که سال‌ها در بازار بوده‌اند رقابت کند؟",
      a: "برندهای قدیمی معمولاً از مدل هزینه‌های سنگین و بسته‌بندی‌های بزرگ استفاده می‌کنند. شما مجبور می‌شوید برای یک قوطی بزرگ هزینه زیادی پرداخت کنید و در نتیجه احساس می‌کنید باید برای مدت طولانی به همان برند وفادار بمانید. ما این مدل را تغییر داده‌ایم. هزینه قوطی‌های پلاستیکی گران، پیمانه‌های گم‌شده و حاشیه سود بالای توزیع‌کنندگان را حذف کرده‌ایم. با استفاده از ساشه‌های روزانه مقاوم در برابر شرایط آب‌وهوایی، کراتین باکیفیت و آزمایش‌شده توسط NABL را با هزینه‌ای مناسب در اختیار مصرف‌کنندگان قرار داده‌ایم. ما فقط یک مکمل نمی‌فروشیم؛ بلکه یک مدل توزیع پرهزینه و گران را تغییر می‌دهیم.",
    },
  ],
};
...
export default function FaqSection() {
  ...
  const content = SECTION_CONTENT[language];

  return (
    <section
      dir={isRTL ? "rtl" : "ltr"}
      className="relative w-full overflow-hidden bg-[#fdf1da] px-3 pb-14 pt-[50px] text-[#3a2416] min-[401px]:px-4 sm:px-10 sm:pb-20 sm:pt-[70px]"
    >
      <div className="mx-auto w-full max-w-4xl">
        <div className="w-full max-w-[940px]">
          <h2
            className={`${anton.className} m-0 text-[clamp(2.2rem,11vw,3rem)] leading-[0.95] text-[#502300] 
            min-[401px]:text-[clamp(2.5rem,11vw,3.75rem)] sm:text-[3.75rem] md:text-[4.375rem] mb-1`}
          >
            {content.first}
          </h2>

          <div className="mt-[-4px] inline-block max-w-full rotate-[2.27deg] bg-[#a87847] px-[0.65rem] py-1 min-[401px]:px-[0.9rem] sm:px-8">
            <h2
              className={`${anton.className} m-0 text-[clamp(2rem,10vw,2.7rem)] leading-[0.95] text-white min-[401px]:text-[clamp(2.25rem,10vw,3.5rem)] sm:text-[3.75rem] md:text-[4.375rem]`}
            >
              {content.second}
            </h2>
          </div>
            ...
            return (
              <div key={item.q} className="w-full">
                <button
                  ...
                  className={`${anton.className} flex w-full min-w-0 cursor-pointer items-center justify-between gap-3 rounded-md border-0 px-[0.7rem] py-[0.85rem] text-left text-[0.85rem] leading-[1.2] text-white transition-colors duration-300 min-[401px]:px-3 min-[401px]:text-[0.9rem] sm:px-5 sm:py-5 sm:text-2xl`}
                  style={{
                ...
                >
                  ...
                  <span className="ml-2 flex h-10 w-10 flex-none items-center justify-center rounded border-2 border-white min-[401px]:h-11 min-[401px]:w-11 sm:ml-4 sm:h-14 sm:w-14">
                    {isOpen ? (
                      <X className="h-4 w-4 min-[401px]:h-[1.1rem] min-[401px]:w-[1.1rem] sm:h-7 sm:w-7" />
                    ) : (
                      <Plus className="h-4 w-4 min-[401px]:h-[1.1rem] min-[401px]:w-[1.1rem] sm:h-7 sm:w-7" />
                    )}
                  </span>
                </button>

                <div
                  className={`grid overflow-hidden rounded-b-md bg-[#bd966e] transition-[grid-template-rows,opacity] duration-[900ms] ease-in-out ${
                    isOpen
                ...
                >
                  <div className="min-h-0 overflow-hidden">
                    <div className="px-[0.85rem] py-3 text-[0.8rem] leading-[1.5] text-[#3a2416] min-[401px]:px-4 min-[401px]:py-3.5 min-[401px]:text-[0.85rem] sm:px-5 sm:py-4 sm:text-base sm:leading-[1.625]">
                      {item.a}
                    </div>
                  </div>
                </div>
